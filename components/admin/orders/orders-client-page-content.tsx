...
export function OrdersClientPageContent() {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            // Solo actualizamos el total para los KPIs si no hay filtros activos
            if (filters.status === 'all' && filters.type === 'all' && !filters.search && !filters.dateFrom && !filters.dateTo) {
    ...
    // Fetch inicial y cuando los filtros cambian (con debounce)
    useEffect(() => {
    ...
    const overviewData = useMemo(() => {
        const sourceData = allOrders; // Los KPIs siempre se calculan sobre el total
